...
import { useProductTaxAssignment } from '../hooks/useProductTaxAssignment';

interface ProductTaxAssignmentDialogProps {
  hook: ReturnType<typeof useProductTaxAssignment>;
}

const ProductTaxAssignmentDialog: React.FC<ProductTaxAssignmentDialogProps> = ({ hook }) => {
  const {
    isOpen,
    loading,
    saving,
  ...
  return (
    <Modal isOpen={isOpen} onClose={close} title={`Manage Taxes – ${product?.name || ''}`} size="lg">
      {loading ? (
